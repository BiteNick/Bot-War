...
public class SniperRiffleScript : Gun
{
    override protected void FireWithDelay()
    {
        if (delay <= 0)
        {
            ...
            {
                ray.direction = Quaternion.Euler(0, Random.Range(-spread + character.spreadDebuff, spread - character.spreadDebuff), 0) * transform.forward * -1;
            }
            else
            {
                ray.direction = Quaternion.Euler(0, 0, 0) * transform.forward * -1;
            }
